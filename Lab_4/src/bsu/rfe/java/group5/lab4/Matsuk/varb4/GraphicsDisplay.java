...
public class GraphicsDisplay extends JPanel {
    ...
    private final BasicStroke graphics_stroke;
    private final BasicStroke axis_Stroke;
    private final BasicStroke grid_stroke;
...
// Шрифт для подписей осей координат
        axisFont = new Font("Times New Roman", Font.BOLD, 36);
...
// Задать треугольник по центру и диагонали
            canvas.setColor(Color.BLACK);
...
//                System.out.`println(Arrays.toString(point));
    ...
    protected void paintAxis(Graphics2D canvas) {
        canvas.setStroke(this.axis_Stroke);
        canvas.setColor(Color.BLACK);
    ...
    private void paintGrid(Graphics2D canvas) {
        canvas.setStroke(this.grid_stroke);
